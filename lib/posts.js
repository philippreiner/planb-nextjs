...
export function getAllArticleIds() {
    const fileNames = fs.readdirSync(ARTICLE_DIRECTORY)
    return fileNames.map(fileName => {
        return {
            params: {
                id: fileName.replace(/\.mdx$/, '')
...
export async function getArticleData(id) {
    const fullPath = path.join(ARTICLE_DIRECTORY, `${id}.mdx`)
    const fileContents = fs.readFileSync(fullPath, 'utf8')
...
export async function getAllCompanyMembers() {
    const fileNames = fs.readdirSync(PEOPLE_DIRECTORY);
    const namesOnly = fileNames.map(fileName => fileName.replace(/\.mdx$/, ''));

    const people = [];
    namesOnly.forEach(async (name) => people.push(await getAuthorData(name)));
